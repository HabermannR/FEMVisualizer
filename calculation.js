...
export function calculateDMatrix(materialProps = defaultMaterialProps) {
    ...
    return [
        [factor,        factor * nu,   0                    ],
        [factor * nu,   factor,        0                    ],
        [0,             0,             factor * (1.0 - nu) / 2.0]
    ];
    // Note: For Plane Strain, use E/(1+nu)/(1-2nu) and different terms.
}
 ...
 */
export function calculateStiffnessMatrix(nodes, materialProps = defaultMaterialProps, quadratureRuleName = '2x2', hourglassAlpha = 0.01) {
    ...
    const thickness = typeof materialProps.t === 'number' ? materialProps.t : 1.0;
    ...
    const D = calculateDMatrix(materialProps); // Constitutive matrix
        ...
        try {
            ...
            for (let i = 0; i < matrixSize; i++) {
                for (let j = 0; j < matrixSize; j++) {
                    if (isNaN(standardContribution[i][j])) {
                        console.error(`[GP ${index+1}] NaN value encountered in standard stiffness contribution at (xi=${xi.toFixed(4)}, eta=${eta.toFixed(4)}). Setting to 0.`);
            ...
            contributions.push({
                ...
                error: nanEncountered ? "NaN in calculation" : null
            });
    ...
    if (quadratureRuleName === '1x1' && hourglassAlpha > 0 && contributions.length > 0 && !contributions[0].error) {
        ...
        try {
            ...
            // 1. Calculate derivatives and Jacobian AT THE CENTER using standard functions
            ...
            // Use the determinant already calculated if available (it's the same point)
            // const detJ_center = contributions[0].detJ;
            ...
            if (detJ_center <= 1e-12) {
                ...
                // 3. Calculate the geometric projection vector 'h' (MATLAB: xi)
                //    h_x = sum(gamma_base[a] * x_a), h_y = sum(gamma_base[a] * y_a)
                const h_vector = [0.0, 0.0]; // [h_x, h_y]
                for (let a = 0; a < nNodes; a++) { // Loop over nodes
                    h_vector[0] += gamma_base[a] * nodes[a].x; // Sum gamma[a] * x[a]
                    h_vector[1] += gamma_base[a] * nodes[a].y; // Sum gamma[a] * y[a]
                }

                // 4. Orthogonalize the base gamma vector
                //    gamma_corrected[a] = gamma_base[a] - sum_{i=x,y} ( h_i * dNa/di )
                let gamma_corrected = [...gamma_base]; // Start with a copy
                // Also calculate the 'scale' factor used in the MATLAB-like stabilization
                // scale = sum_{a=node} sum_{i=coord} (dNa/di * dNa/di) at the center
                ...
                for (let a = 0; a < nNodes; a++) { // Loop over nodes
                    ...
                    correction_term += h_vector[0] * dNdx_a; // h_x * dNa/dx
                    correction_term += h_vector[1] * dNdy_a; // h_y * dNa/dy

                    gamma_corrected[a] -= correction_term; // Apply correction

                    // Accumulate scale factor (sum of squares of derivatives at center)
                    scale_factor += dNdx_a * dNdx_a + dNdy_a * dNdy_a;
                }

                // 5. Calculate the hourglass stiffness contribution magnitude
                // Formula mimic: K_hg = alpha * scale * gamma_corrected * gamma_corrected^T * Area
                // Note: The 'scale' factor here is specific to the referenced MATLAB method.
                //       Other HG methods might use E or G directly.
                const Area = detJ_center * 4.0; // Area = detJ * weight (weight=4 for 1x1)
                const stabFactor = hourglassAlpha * scale_factor * Area * thickness; // Include thickness

                // 6. Assemble K_hg (8x8)
                //    K_hg_ij = stabFactor * gamma_corrected[a] * gamma_corrected[b] for corresponding DOFs
                for (let a = 0; a < nNodes; a++) { // Node index 'a' (row node)
                    for (let b = 0; b < nNodes; b++) { // Node index 'b' (column node)
                        // Calculate global DOF indices in the 8x8 matrix
                        let row_x = nDofNode * a + 0; // x-dof index for node a
                        let row_y = nDofNode * a + 1; // y-dof index for node a
                        let col_x = nDofNode * b + 0; // x-dof index for node b
                        let col_y = nDofNode * b + 1; // y-dof index for node b

                        const gamma_ab_term = stabFactor * gamma_corrected[a] * gamma_corrected[b];

                        // Add contribution: Assumes stabilization affects x and y DOFs identically
                        // based on the single gamma vector (common for this 'scaled' type)
                        K_hg[row_x][col_x] += gamma_ab_term;
                        K_hg[row_y][col_y] += gamma_ab_term;
                        // No cross-terms (x-y) are added by this specific stabilization type
                    }
                }
                ...
                for (let i = 0; i < matrixSize; i++) {
                    for (let j = 0; j < matrixSize; j++) {
                         if (isNaN(K_hg[i][j])) {
                             console.error(`   Hourglass Control Error: NaN value encountered in K_hg calculation. Setting to 0.`);
                             K_hg[i][j] = 0;
                             nanInHg = true;
                         }
                         K[i][j] += K_hg[i][j];
                 ...
                 hgDetails = {
                    ...
                    Area: Area,
                    stabFactor: stabFactor,
                    K_hg: K_hg, // The actual added HG stiffness matrix
                    error: nanInHg ? "NaN in K_hg calculation" : null
                 };
...
export function calculateDisplacements(nodes, forceArrow, materialProps, quadratureRuleName = '2x2', hourglassAlpha = 0.01) {
    console.log(quadratureRuleName)
    // Example Fixed DOFs
     const fixedDOFs = [ 5, 6, 7 ]; // Matches reducedStiffnessMatrix example
     const matrixSize = 8;

    // --- Create Full Force Vector ---
    const F = new Array(matrixSize).fill(0);
    if (forceArrow && typeof forceArrow.length === 'number' && typeof forceArrow.angle === 'number') {
        const Fx = forceArrow.length * Math.cos(forceArrow.angle);
        const Fy = forceArrow.length * Math.sin(forceArrow.angle);
         // Apply force to Node 2 (DOFs 2, 3) - Adjust if force node changes
         F[2] = Fx;
         F[3] = Fy;
    } else {
        console.warn("Invalid or missing forceArrow object. Applying zero force.");
    }
    ...
    const Kr = reducedStiffnessMatrix(nodes, materialProps, quadratureRuleName, hourglassAlpha);
    const Fr = freeDOFs.map(dof => F[dof]); // Extract forces corresponding to free DOFs


    // --- Solve for Free Displacements ---
    console.log("Solving system Kr * Ur = Fr");
    const Ur = solveSystem(Kr, Fr); // Solve the reduced system

    // --- Check Solver Result ---
    if (!Ur || Ur.some(u => isNaN(u) || !isFinite(u))) {
        console.error("Solver failed or produced non-finite displacements. Stiffness matrix might be ill-conditioned or singular due to inadequate constraints or element distortion. Returning zero displacements.");
        // Return zero displacements to prevent downstream errors
        return Array(nodes.length).fill({ x: 0, y: 0 });
    }


    // --- Assemble Full Displacement Vector ---
    const U = new Array(matrixSize).fill(0); // Initialize full displacement vector with zeros
    freeDOFs.forEach((dof, i) => {
        U[dof] = Ur[i]; // Place calculated free displacements
    });
    console.log("Full Displacement Vector U:", U);

    // --- Format Output ---
    const displacements = [];
    for (let i = 0; i < nodes.length; i++) {
        displacements.push({ x: U[2 * i], y: U[2 * i + 1] });
    }
